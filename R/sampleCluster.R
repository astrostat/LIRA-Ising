...
sampleCluster<-function(z_cluster,
  ...
  hz_plus.log<-sum(hzParam(rep(1, nrow(z_cluster)), z_cluster$lambda, tau0, tau1, sigma0_sq, sigma1_sq))
  hz_minus.log<-sum(hzParam(rep(-1, nrow(z_cluster)), z_cluster$lambda, tau0, tau1, sigma0_sq, sigma1_sq))
  ...
  return(rep(new.spin, nrow(z_cluster)))
